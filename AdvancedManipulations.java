...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        int avg = (int) Math.round(sum / (3 * w * h));
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                m[row][col] = new Pixel(red, green, blue);
            }
        }
        return new PixelPicture(m);
    }
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        // obtain sorted Array
        Pixel[] picArr = picMap.getSortedPixels();
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                Pixel close = picArr[0];
                for (int i = 0; i < numColors; i++) {
                    if (p.distance(picArr[i]) <= diff) {
                        if (p.distance(picArr[i]) == diff) {
                            if (picMap.getValue(picArr[i]) > picMap.getValue(close)) {
                                close = picArr[i];
                            }
                        } else {
                            diff = p.distance(picArr[i]);
                            close = picArr[i];
                        }
                    }
                }
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        // get width, height, and bitmap and store into respective variables
        int w = pic.getWidth();
        int h = pic.getHeight();

        // create two pixels, one that will be the final (fin) drawing
        Pixel[][] orig = pic.getBitmap();
        Pixel[][] fin = new Pixel[h][w];

        /*
         * establish the leftmost and rightmost bounds, then test
         * to make sure it's within our bounds
         */
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                // find pixel average from that specific region
                Pixel picAvg = avg(h, w, row, col, radius, orig);
                // set the average pixel to the one we want
                fin[row][col] = picAvg;
            }
        }
        // return new pixelpicture modified
        return new PixelPicture(fin);
    }

    // NOTE: You may want to add a static helper function here to
    // help find the average color around the pixel you are blurring.
    public static Pixel avg(int h, int w, int x, int y, int radius, Pixel[][] bmp) {

        // average doubles for red, green, and blue
        int avgRed = 0;
        int avgGreen = 0;
        int avgBlue = 0;
        int count = 0;

        // obtain values from bitmap, but only the ones we want
        for (int row = -radius; row <= radius; row++) {
            for (int col = -radius; col <= radius; col++) {
                if ((row + x) > -1 && (row + x) < h
                        && (col + y) > -1 && (col + y) < w) {
                    avgRed += bmp[row + x][col + y].getRed();
                    avgGreen += bmp[row + x][col + y].getGreen();
                    avgBlue += bmp[row + x][col + y].getBlue();
                    count++;
                }
            }
        }
        // take the average of the picture, dimensions = divisor
        avgRed = (int) Math.round(avgRed / count);
        avgGreen = (int) Math.round(avgGreen / count);
        avgBlue = (int) Math.round(avgBlue / count);

        // return new pixel with average colors
