...
public class SimpleManipulations {
    ...
    public static PixelPicture rotateCCW(PixelPicture pic) {
        // get width and height of pixel picture
        int w = pic.getWidth();
        int h = pic.getHeight();
    ...
    public static PixelPicture border(
        ...
        // create two pixel arrays: a new one for border and old one, our source
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[w + 2 * borderWidth][h + 2 * borderWidth];

        // top
        for (int col = 0; col < borderWidth; col++) {
            for (int row = 0; row < 2 * borderWidth + w; row++) {
                // set to border color
        ...
        for (int col = 0; col < borderWidth; col++) {
            for (int row = 0; row < 2 * borderWidth + w; row++) {
                // set to border color
                tgt[row][borderWidth + h + col] = borderColor;
            }
        }
        ...
        for (int row = 0; row < borderWidth; row++) {
            for (int col = 0; col < h + 2 * borderWidth; col++) {
                // set to border color
                tgt[w + borderWidth + row][col] = borderColor;
            }
        }
        ...
        for (int row = 0; row < w; row++) {
            for (int col = 0; col < h; col++) {
                // set our source image to the target, no borders graphed here
    ...
    public static PixelPicture grayScaleLuminosity(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                m[row][col] = new Pixel(avg, avg, avg);
            }
        }
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                m[row][col] = new Pixel(255 - red, 255 - green, 255 - blue);
            }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                int newCol = (int) Math.round(gray);
                m[row][col] = new Pixel(newCol, newCol, newCol);
            }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                int blue = p.getBlue();
                int green = p.getGreen();
                double red1 = red * rfactor;
                int newRed = (int) Math.round(red1);
                double blue1 = blue * bfactor;
                int newBlue = (int) Math.round(blue1);
                double green1 = green * gfactor;
                int newGreen = (int) Math.round(green1);
                m[row][col] = new Pixel(newRed, newGreen, newBlue);
            }
        ...
        if (w1 == w2 && h1 == h2) {
            for (int row = 0; row < h1; row++) {
                for (int col = 0; col < w1; col++) {
                    ...
                    // new colors, a weighted average
                    int newRed = weightedAverage(alpha, r1, r2);
                    int newBlue = weightedAverage(alpha, b1, b2);
                    int newGreen = weightedAverage(alpha, g1, g2);
                    m[row][col] = new Pixel(newRed, newGreen, newBlue);
                }
            }
            return new PixelPicture(m);
